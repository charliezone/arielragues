...
if ( !class_exists('EG_Model') ) {
    ...
    class EG_Model {
        ...
        function get_galleries() {
            ...
            $galleries = $this->egwpdb->get_results(sprintf("SELECT * FROM `%s` ORDER BY `gallery_title` ASC", EG_GALLERY_TABLE), ARRAY_A);
            return $galleries;
        }
        ...
        function get_gallery_row($gallery_id) {
            $gallery_row = $this->egwpdb->get_row(sprintf("SELECT * FROM `%s` WHERE gallery_id = %d", EG_GALLERY_TABLE, $gallery_id), ARRAY_A);
            return $gallery_row;
        }
        ...
        function update_gallery($received_data) {
            ...
            //  EG_Library::print_array($received_data);
            // die();
        ...
        function delete_gallery($gallery_id) {
            $delete_check = $this->egwpdb->delete(EG_GALLERY_TABLE, array( 'gallery_id' => $gallery_id ), array( '%d' ));
            return $delete_check;
        }
        ...
        function get_gallery_row_by_alias($alias = '') {
            $gallery_row = $this->egwpdb->get_row(sprintf("SELECT * FROM `%s` WHERE gallery_alias = '%s'", EG_GALLERY_TABLE, $alias), ARRAY_A);
            return $gallery_row;
        }

    }

    $GLOBALS[ 'egwpdb' ] = new EG_Model;
}